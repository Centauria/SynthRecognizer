...
class ConformerEncoder(Conformer):
    """
    A wrapper for conformer using as an encoder of transformer.
    """

    def forward(self, input: torch.Tensor,
                mask: Optional[torch.Tensor] = None,
                src_key_padding_mask: Optional[torch.Tensor] = None) -> Tuple[torch.Tensor, torch.Tensor]:
        x, _ = super(ConformerEncoder, self).forward(input,
                                                     torch.tensor(input.size(1)).repeat(input.size(0)).to(input.device))
        return x
